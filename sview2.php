...
require_once('connect.php');
  ...
  if (isset($_SESSION['user_name'])) { 
    ...
    $query1= "SELECT comment,sname FROM icomment WHERE insid = '" . $id. "'";
  }
 // else {
       ...
       // require_once('connect.php');
        ...
        //$data2=mysqli_query($d1,$query2);
         ...
         // $user=$row2['username'];
          ...
          $query3="insert into icomment(insid,sname,comment) values ('$u','$n','$c')";
          $data3=mysqli_query($d1,$query3);

          if($data3)
          {
            echo "<script type='text/javascript'>alert('your response has been recorded!!!');location.href='sview2.php'</script>";
          }
          else
          {
    ...
    $query4="SELECT name,email,phone from student where username='".$_SESSION['user_name']."'";
        $data4=mysqli_query($d1,$query4);
         $row4=mysqli_fetch_array($data4);
          ...
          $p1=$row4['phone'];
          //echo $n1;
    $query5="insert into ireq(insid,sname,email,phone,time) values('$u1','$n1','$e1','$p1',now())";
