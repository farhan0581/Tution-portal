...
<p>
 ...
 value="<?php if (!empty($row['t2'])) echo $row['t2'] ; ?>"></h1>
    ...
    <div class="wrapper" >
                    <div class="text-form" color="black"></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <textarea rows="4" cols="40" color="black" name="comments"></textarea>
                    <br><br>
                    <p align="">  <input style="float:left; margin-left:30px; margin-bottom:10px;"type="submit" class="btn btn-primary" name="comment" value="comment"  ></p>
                  </div>
       ...
       // require_once('connect.php');
        ...
        //$data2=mysqli_query($d1,$query2);
         ...
         // $user=$row2['username'];
          ...
          $query3="insert into tcomment(tuid,sname,comment) values ('$u','$n','$c')";
          $data3=mysqli_query($d1,$query3);

          if($data3)
          {
            echo "<script type='text/javascript'>alert('your response has been recorded!!!');location.href='sview1.php'</script>";
          }
          else
          {
...
{
  echo "<script type='text/javascript'>alert('Sorry you are not student!!');</script>";
}
}

if (isset($_POST['request']))
{
  if($_SESSION['type']=='student'||$_SESSION['type']=='institute')
  {
    $u1=$_SESSION['id1'];
    $query4="SELECT name,email,phone from student where username='".$_SESSION['user_name']."'";
        $data4=mysqli_query($d1,$query4);
         $row4=mysqli_fetch_array($data4);
          ...
          $p1=$row4['phone'];
          //echo $n1;
    $query5="insert into treq(tuid,sname,email,phone,time) values('$u1','$n1','$e1','$p1',now())";
...
echo "<script type='text/javascript'>alert('Sorry you are not student/institute!');</script>";
}
