...
<?php
 ...
 @session_start();
  ...
  if (isset($_SESSION['user_name'])) { 
    $query = "SELECT username, name, email, subjects, fee, location, city,state,phone,t1,t2 FROM institute WHERE username = '" . $_SESSION['user_name'] . "'";
  }
 // else {
...
<div class="row-2">
                      <nav>
                        <ul class="menu">
                          ...
                          <li><a class="active" href="institute.php">For Institute</a></li>
...
require_once('footer.php');
 if (isset($_POST['eprof'])) {
    ...
    // Update the profile data in the database
       if(!empty($username)&&!empty($email)&&!empty($name)&&!empty($sub)&&!empty($city)&&!empty($state)&&!empty($fe)&&!empty($loc)&&!empty($number)&&!empty($t1)&&!empty($t2)) {
        
           $query1= "UPDATE log SET username = '$username'  WHERE username = '" . $_SESSION['user_name'] . "'";
          $query2 = "UPDATE institute SET username = '$username', name = '$name', email = '$email', " .
            " city = '$city', state = '$state',fee='$fe', subjects = '$sub', location='$loc',phone='$number',t1='$t1',t2='$t2' WHERE username = '" . $_SESSION['user_name'] . "'";
       ...
       $result = mysqli_query($d1, $query2);
        ...
        if($result)
        {
            echo "<script type='text/javascript'>alert('Your profile has been updated!'); location.href='iview.php'</script>"; 
        }
        else
          echo"problre";
       ...
       // mysqli_close($d1);
        ...
        echo "<script type='text/javascript'>alert('enter all the fileld!'); location.href='iview.php'</script>";
      }
