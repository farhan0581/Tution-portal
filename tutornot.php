...
<?php 
  ...
  // require_once('connect.php');
          ...
          {
            echo "<script type='text/javascript'>alert('You are not tutor!!');location.href='tutor.php'</script>";
            exit();
          }
   $qu="select tuid from tutor where username='".$_SESSION['user_name']."'";
   $dt=mysqli_query($d1,$qu);
   $r1=mysqli_fetch_array($dt);
   $id=$r1['tuid'];
          $rs=mysqli_query($d1,"SELECT COUNT(sname) from treq where tuid='".$id."'");
...
@session_start();
   ...
   $id=$r1['tuid'];
                                ...
                                $rs_result = mysqli_query ($d1,"SELECT sname,email,phone,time from treq where tuid='".$id."' LIMIT $start_from, 3");
                               ...
                               // $_SESSION['id1']=$row['tuid'];
                                ...
                                ?>
                                        ...
                                        <?php
                                $result1 = mysqli_query($d1,"SELECT COUNT(sname) from treq where tuid='".$id."'");
                               ...
                               // $f= $total_records ;
                                ...
                                for ($i=1; $i<=$f; $i++) {
                                            //echo '<button';
                                            echo "<a href='tutornot.php?page=".$i."'>   " . $i. "</a>";
                                            //echo '</footer>';
                                }
